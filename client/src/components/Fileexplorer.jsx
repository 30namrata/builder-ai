import { FileCodeIcon, FileIcon, FileTextIcon, FolderOpenIcon } from "lucide-react";
import { useMemo } from "react";


const buildtree = ((file) => {
    const root = [];
    for (const filePath of file.sort()) {
        const parts = filePath.split("/").filter(Boolean);
        let current = root;
        for (let i = 0; i < parts.length; i++) {
            const name = parts[i];
            const isLast = i === parts.length - 1;
            const isFull = "/" + parts.slice(0, i + 1).join("/");
...
function TreeItem({ node, activeFile, onFileSelect, depth = 0, }) {
    ...
    return (
        <button
            ...
            className={`w-full flex items-center gap-2 py-1.5 px-2 text-xs transition-colors rounded-md cursor-pointer ${isActive
...
const FileExplorer = ({ file, activeFile, onFileSelect }) => {
    const fileTree = useMemo(() =>
        buildtree(Object.keys(file)), [file])
